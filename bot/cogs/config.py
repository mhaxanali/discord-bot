...
def has_permissions_or_owner(**perms):
    """
    Returns a command check that passes if the invoking user is the bot owner
    or if they have all of the specified Discord permissions.
    """
...
class Config(commands.Cog):
    ...
    async def cog_check(self, ctx: commands.Context) -> bool:
        """
        Applies lock/hard-lock gating to every command except read-only
        and lock/unlock commands.
        """
        if ctx.guild is None:
            return False

        # Bot owner bypasses all config locks.
        if ctx.author.id == BOT_OWNER_ID:
            return True

        # These commands are always available regardless of lock state.
        ...
        if cfg["is_locked"] and ctx.author.id != ctx.guild.owner_id:
            await ctx.send(
                embed=error_embed(
                    "This server's config is locked. Only the server owner "
                    "can make changes."
                )
            )
            return False
    ...
    async def config(self, ctx: commands.Context) -> None:
        ...
        modlogs_channel = (
            f"<#{cfg['mod_logs_channel']}>"
            if cfg["mod_logs_channel"]
            else "Not set"
        )
        ...
        enabled_cogs = (
            ", ".join(f"`{c}`" for c in cfg["enabled_cogs"])
            or "None"
        )
        ...
        embed.add_field(
            ...
            value=modlogs_channel,
            inline=True,
        )

        embed.add_field(
            name="Mod Logs Enabled",
            value=modlogs_enabled,
            inline=True,
        )
        ...
        embed.set_footer(
            text="Run +config help for a list of commands."
        )
    ...
    async def config_help(self, ctx: commands.Context) -> None:
        ...
        for command in sorted(
            self.config.walk_commands(),
            key=lambda c: c.qualified_name,
        ):
            doc = (command.help or "No description.").split("\n")[0]
            lines.append(
                f"`+{command.qualified_name}` - {doc}"
            )
    ...
    async def cogs_enable(
        ...
        if cog_name in DEFAULT_COGS:
            await ctx.send(
                embed=error_embed(
                    f"`{cog_name}` is a default cog and is always enabled."
                )
            )
            return
        ...
        if missing:
            await ctx.send(
                embed=error_embed(
                    f"Bot is missing required permissions for "
                    f"`{cog_name}`: "
                    + ", ".join(missing)
                )
            )
        ...
        await self.bot.connector.set_enabled_cogs(
            ctx.guild.id,
            updated,
        )
    ...
    async def cogs_disable(
        ...
        if cog_name in DEFAULT_COGS:
            await ctx.send(
                embed=error_embed(
                    f"`{cog_name}` is a default cog and cannot be disabled."
                )
            )
            return
    ...
    async def tod_channel(
        ...
        *,
        ...
        if channel is None:
            await ctx.send(
                embed=error_embed(
                    "Provide a channel mention/ID, or `reset` to clear it."
                )
            )
            return

        if channel.lower() == "reset":
            await self.bot.connector.set_tod_channel(
                ctx.guild.id,
                None,
            )
            ...
            await ctx.send(
                embed=success_embed(
                    "TOD channel reset. Game usable in any channel."
                )
            )
            return

        try:
            resolved = await commands.TextChannelConverter().convert(
                ctx,
                channel,
            )
        ...
        await ctx.send(
            embed=success_embed(
                f"TOD channel set to {resolved.mention}."
            )
        )
    ...
    async def tod_role(
        ...
        *,
        ...
        if role is None:
            await ctx.send(
                embed=error_embed(
                    "Provide a role mention/ID, or `reset` to clear it."
                )
            )
            return

        if role.lower() == "reset":
            await self.bot.connector.set_tod_role(
                ctx.guild.id,
                None,
            )
            ...
            await ctx.send(
                embed=success_embed(
                    "TOD role reset. No role will be pinged."
                )
            )
            return

        try:
            resolved = await commands.RoleConverter().convert(
                ctx,
                role,
            )
        ...
        await ctx.send(
            embed=success_embed(
                f"TOD role set to {resolved.mention}."
            )
        )
    ...
    async def modlogs_channel(
        ...
        *,
        ...
        if channel.lower() == "reset":
            await self.bot.connector.set_mod_logs_channel(
                ctx.guild.id,
                None,
            )
        ...
        await ctx.send(
            embed=success_embed(
                f"Mod logs channel set to {resolved.mention}."
            )
        )
    ...
    async def modlogs_enable(
        ...
        if cfg["mod_logs_channel"] is None:
            await ctx.send(
                embed=error_embed(
                    "Set a mod logs channel first with "
                    "`+config modlogs channel`."
                )
            )
            return

        await self.bot.connector.set_enable_mod_logs(
            ctx.guild.id,
            True,
        )
    ...
    async def config_lock(self, ctx: commands.Context) -> None:
        ...
        if (
            ...
            and ctx.author.id != BOT_OWNER_ID
        ):
            await ctx.send(
                embed=error_embed(
                    "Only the server owner can lock the config."
                )
            )
            return

        await self.bot.connector.set_locked(
            ctx.guild.id,
            True,
        )
        ...
        await ctx.send(
            embed=success_embed(
                "Config locked. Only the server owner can make changes."
            )
        )

    @config_lock.command(name="hard")
    async def config_lock_hard(
        self,
        ctx: commands.Context,
    ) -> None:
        ...
        if ctx.author.id != BOT_OWNER_ID:
            await ctx.send(
                embed=error_embed(
                    "Only the bot owner can hard-lock a config."
                )
            )
            return

        await self.bot.connector.set_hard_locked(
            ctx.guild.id,
            True,
        )
        ...
        await ctx.send(
            embed=success_embed(
                "Config hard-locked. Only the bot owner can make changes now."
            )
        )
    ...
    async def config_unlock(self, ctx: commands.Context) -> None:
        ...
        if (
            ...
            await ctx.send(
                embed=error_embed(
                    "Only the server owner can unlock the config."
                )
            )
            return
    ...
    async def config_unlock_hard(
        ...
        if ctx.author.id != BOT_OWNER_ID:
            await ctx.send(
                embed=error_embed(
                    "Only the bot owner can remove a hard lock."
                )
            )
            return
